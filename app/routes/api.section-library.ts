...
export async function action({ request }: ActionFunctionArgs) {
  ...
  try {
    ...
    if (intent === "save_builder_section") {
      ...
      const syncMessage = savedSection.nativeThemeSyncEnabled
        ? savedSection.syncedToTheme
          ? savedSection.addedToHomepage
            ? "Saved to the active theme and inserted into the Home template."
            : "Saved to the active theme. It can now appear in Add section under ShopBuilder."
          : savedSection.syncError ||
            "Saved in ShopBuilder, but Shopify did not allow the native theme sync."
        : `Saved "${name}" in ShopBuilder. Open Theme Editor and choose it by name.`;
      ...
      return new Response(
        JSON.stringify({
          ...
          syncedToTheme: savedSection.syncedToTheme,
          syncMessage,
          syncError: savedSection.syncError,
          nativeThemeSyncEnabled: savedSection.nativeThemeSyncEnabled,
          themeEditorUrl: savedSection.nativeThemeSyncEnabled
            ? buildThemeEditorUrl(session.shop)
            : null,
    ...
    return new Response(
      JSON.stringify({
        error:
          error?.message ||
          "Shopify rejected the save request while syncing the section.",
      }),
      {
        status: 500,
        headers: { "Content-Type": "application/json" },
